...
describe("SHIBAINUExample ERC20 Standard Test", function () {
  ...
  describe("Contract Initialization", function () {
    ...
    it("Should correctly report the token balance of the contract owner", async function () {
      ...
      const supply = ethers.utils.parseUnits("1000000000000000", decimals).sub("2000");
    ...
    it("Should correctly report the token total supply", async function () {
      ...
      expect(supply).to.equal(ethers.utils.parseUnits("1000000000000000", decimals));
    })
  });
  ...
  describe("Token Transfer Functionality", function () {
    ...
    it("Should revert when sender does not have sufficient balance for transfer", async function () {
      const decimals = await token.decimals(); 
      await expect(token.connect(recipient3).transfer(owner.address, ethers.BigNumber.from("1000000000000000").mul(ethers.BigNumber.from("10").pow(decimals)).add("1"))).to.be.revertedWith("ERC20: transfer amount exceeds balance");
    });
    });
  ...
  describe("Token transferFrom Functionality", function () {
    ...
    it("Should revert when sender does not have a sufficient balance on transferFrom function", async function () {
      await expect(token.connect(recipient1).transferFrom(recipient2.address, owner.address, 1000000000000001)).to.be.revertedWith("ERC20: insufficient allowance");
    });
  });
